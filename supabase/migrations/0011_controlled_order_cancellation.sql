...
begin
  ...
  if target.status in ('delivered', 'cancelled') then raise exception 'This order can no longer be cancelled'; end if;
  update public.orders set status = 'cancelled', updated_at = now() where id = order_id;
  return order_id;
end;
$$;
grant execute on function public.cancel_market_order(uuid) to authenticated;
